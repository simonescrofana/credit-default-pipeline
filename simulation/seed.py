...
def validated(schema_cls: type[BaseResponseSchema], **data) -> dict:
    """Validate a raw record against its Pydantic schema before ORM construction.

    Centralizes the "validate, then build" pattern used at every ORM
    instantiation point in this module.

    Args:
        schema_cls (type[BaseResponseSchema]): The Create schema to validate against.
        **data: Raw field values as generated by the simulation layer.

    Returns:
        dict: The validated field values, ready to be unpacked into the corresponding
        SQLAlchemy model.
...
def persist_billing_and_telemetry(
    ...
    for b in billing_history:
        ...
        invoice_data = {
            ...
            "invoice_number": f"""INV-{comm_type[:3].upper()}-
        ...
        inv = Invoice(**validated(InvoiceCreate, **invoice_data))
        session.add(inv)
        session.flush()
        ...
        if b["pay_days"] is not None:
            payment_data = {
                "invoice_id": inv.id,
                "payment_date": inv.issue_date + datetime.timedelta(days=b["pay_days"]),
                "amount_paid": inv.total_amount,
                "payment_method": b["payment_method"],
                "transaction_reference": f"""TRX-
                    {fake.unique.bothify(text="?#?#?#?#?#?#")}""",
                "payment_status": "completed"
                if b["payment_behavior"] == "completed"
                else "failed",
            }
            session.add(Payment(**validated(PaymentCreate, **payment_data)))

    for t in tickets:
        session.add(
            CRMSupportTicket(
                **validated(CRMSupportTicketCreate, company_id=company_id, **t)
            )
        )
    for login in logins:
        session.add(
            UserWebLogin(
                **validated(UserWebLoginCreate, company_id=company_id, **login)
            )
        )
...
def generate_company_history(
    ...
    db_company = Company(**validated(CompanyCreate, **comp_data))
    ...
    for cb in contracts_blueprint:
        ...
        db_c = EnergyContract(**validated(EnergyContractCreate, **contract_data))
        session.add(db_c)
        session.flush()
